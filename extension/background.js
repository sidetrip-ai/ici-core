// Background script for ChatGPT Context Enhancer

console.log('ChatGPT Context Enhancer background script started');

// Ping counter to verify extension is running
let pingCount = 0;
const pingInterval = setInterval(() => {
  pingCount++;
  console.log(`ping ${pingCount} (background.js)`);
}, 5000);
...
function checkContentScriptRunning() {
  console.log('Checking if ChatGPT Context Enhancer content script is running...');
  ...
  if (!debugElement) {
    ...
    notificationDiv.textContent = 'ChatGPT Context Enhancer may not be working properly. Please check the console or try reloading.';
